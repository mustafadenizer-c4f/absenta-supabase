  ...
  useEffect(() => {
    ...
    const fetchGroupData = async () => {
      ...
      try {
        ...
        setError(err.message || 'Failed to load group team data');
      } finally {
        setLoading(false);
      }
    };
  ...
  if (!user?.group_id) {
    return (
      <Box>
        ...
        <Alert severity="warning">No group assigned. Please contact an administrator.</Alert>
      </Box>
    );
  }
    ...
    <Box>
      ...
      <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 2, mb: 2, alignItems: 'center' }}>
        ...
        <Autocomplete
          ...
          renderInput={(params) => (
            <TextField {...params} label="Filter by group member" placeholder="Select members" size="small" />
          )}
