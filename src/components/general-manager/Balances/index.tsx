...
const DepartmentBalances: React.FC = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const { data: allBalances, error: bErr } = await supabase
        .from('leave_balances').select('*').in('user_id', memberIds);
      if (bErr) throw bErr;

      const { data: allApproved, error: aErr } = await supabase
        .from('leave_requests')
        .select('user_id, leave_type_id, total_days, start_date')
        .in('user_id', memberIds).eq('status', 'approved')
        .gte('start_date', minStart).lte('start_date', maxEnd);
      if (aErr) throw aErr;

      const { data: allPending, error: pErr } = await supabase
        .from('leave_requests')
        .select('user_id, leave_type_id, total_days, start_date')
        .in('user_id', memberIds).eq('status', 'pending')
        .gte('start_date', minStart).lte('start_date', maxEnd);
      if (pErr) throw pErr;
  ...
  return (
    <Box>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 4 }}>
        <Typography variant="h4" sx={{ color: 'primary.main', fontWeight: 600 }}>Balances</Typography>
        <Button variant="outlined" startIcon={<Refresh />} onClick={fetchData}>Refresh</Button>
      </Box>
      {error && <Alert severity="error" sx={{ mb: 3 }} onClose={() => setError(null)}>{error}</Alert>}
      {rows.length === 0 ? (
        <Paper sx={{ p: 4, textAlign: 'center' }}><Typography color="text.secondary">No department members found.</Typography></Paper>
      ) : (
        <Paper>
          <TableContainer>
            <Table size="small">
