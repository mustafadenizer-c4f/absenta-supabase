...
const DepartmentManagerApprovals: React.FC = () => {
  ...
  const handleActionConfirm = async () => {
    ...
    try {
      ...
      setSnackbar({ open: true, message: isStale ? 'This request is no longer pending. The list has been refreshed.' : msg, severity: 'error' });
      setDialogOpen(false);
      setActionTarget(null);
      setComment('');
      fetchRequests();
    ...
    <Box>
      ...
      <Paper>
        {loading ? (
          ...
          <Box sx={{ p: 4, textAlign: 'center' }}><Typography variant="body1" color="text.secondary">No pending leave requests to review.</Typography></Box>
        ) : (
          <>
            <TableContainer>
              <Table>
                  ...
                  {requests.map((req) => {
                    ...
                    return (
                      <TableRow key={req.id} hover>
                        ...
                        <TableCell>
                          {self ? (
                            <Typography variant="caption" color="text.secondary">Own request</Typography>
                          ) : (
      ...
      <Dialog open={dialogOpen} onClose={handleDialogClose} maxWidth="sm" fullWidth>
        ...
        <DialogContent>
          ...
          <TextField label="Comment (optional)" multiline rows={3} fullWidth value={comment} onChange={(e) => setComment(e.target.value)} placeholder="Add a comment for the employee..." />
        </DialogContent>
        <DialogActions>
